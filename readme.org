...
#+begin_src emacs-lisp
  (use-package org-expand-doi
    :vc (:url "https://github.com/cvanmarcke/org-expand-doi")
    ;; If you use `elpaca':
...
- =org-expand-doi-export-backend=: (Default ='all=) Specifies which org-export backends (='md=, '=html=, ...) should expand doi links.
- =org-expand-doi-replace-existing-description=: (Default =t=) If non-nil, replace the description when expanding doi links. When nil, keep the description as-is.
- =org-expand-doi-make-link=: (Default =t=) Expand into a link when non-nil.
